...
// 定义ModePosLoiter类的init方法，此方法旨在初始化PosHold（位置保持）模式。
// 如果ignore_checks为true，某些检查可能会被忽略。
bool ModePosLoiter::init(bool ignore_checks)
{
...
void ModePosLoiter::update_pilot_lean_angle(float& lean_angle_filtered, float& lean_angle_raw)
{
...
float ModePosLoiter::mix_controls(float mix_ratio, float first_control, float second_control)
{
...
void ModePosLoiter::update_brake_angle_from_velocity(float& brake_angle, float velocity)
{
    float lean_angle;
...
// 将风补偿估算初始化为零
void ModePosLoiter::init_wind_comp_estimate()
{
...
void ModePosLoiter::update_wind_comp_estimate()
{
    // 检查风估算是否延迟启动
...
//  应以最大循环速率调用
void ModePosLoiter::get_wind_comp_lean_angles(float& roll_angle, float& pitch_angle)
{
    // 降低速率为10hz
    wind_comp_timer++;
...
void ModePosLoiter::roll_controller_to_pilot_override()
{
...
void ModePosLoiter::pitch_controller_to_pilot_override()
{
