...
public:
    ...
    enum class Number : uint8_t {
        STABILIZE    = 0,  // 手动控制飞行器角度，手动油门
        ACRO         = 1,  // 手动控制机体角速度，手动油门
        ALT_HOLD     = 2,  // 手动控制飞行器角度，自动油门
        AUTO         = 3,  // 完全自动控制，通过任务指令控制航点
        GUIDED       = 4,  // 完全自动控制，根据地面控制站(GCS)的指令飞向特定坐标或以特定速度/方向飞行
        LOITER       = 5,  // 自动水平加速度控制，自动油门
        RTL          = 6,  // 自动返回起飞点
        CIRCLE       = 7,  // 自动环绕飞行，自动油门
        LAND         = 9,  // 自动降落，水平位置控制
        DRIFT        = 11, // 半自主位置，偏航和油门控制
        SPORT        = 13, // 手动控制地面坐标系的角速度，手动油门
        FLIP         = 14, // 自动翻转飞行器（绕滚转轴）
        AUTOTUNE     = 15, // 自动调整飞行器的横滚和俯仰增益
        POSHOLD      = 16, // 自动位置保持，可手动干预，自动油门
        BRAKE        = 17, // 利用惯性/GPS系统进行全刹车，无需飞行员输入
        THROW        = 18, // 投掷起飞模式，利用惯性/GPS系统，无需飞行员输入
        AVOID_ADSB   = 19, // 自动避免宏观障碍，例如全尺寸飞机
        GUIDED_NOGPS = 20, // 引导模式，但只接受姿态和高度输入
        SMART_RTL    = 21, // SMART_RTL 通过回溯其步骤返回起点
        FLOWHOLD     = 22, // FLOWHOLD 使用光流保持位置，不需要测距仪
        FOLLOW       = 23, // 跟随模式试图跟随另一架飞行器或地面站
        ZIGZAG       = 24, // ZIGZAG 模式能够以预定的点A和点B进行之字形飞行
        SYSTEMID     = 25, // 系统ID模式在控制器中产生自动系统识别信号
        AUTOROTATE   = 26, // 自主自转旋翼模式
        AUTO_RTL     = 27, // 自动RTL，这不是一个真正的模式，如果进入执行DO_LAND_START着陆序列，AUTO将报告为此模式
        TURTLE       = 28, // 坠毁后翻转模式
        POSLOITER    = 29, // 自动位置保持，可手动干预，自动油门
...
private:
    ...
    enum class RPMode {
        PILOT_OVERRIDE = 0,          // 飞行员正在控制此轴（即滚动或俯仰）
        BRAKE,                       // 该轴正在减速至零
        BRAKE_READY_TO_LOITER,       // 该轴已完成减速并准备进入悬停模式（两个轴都必须是此值才能进入下一阶段）
        BRAKE_TO_LOITER,             // 飞行器的两个轴（滚动和俯仰）正在从减速模式过渡到悬停模式（混合减速和悬停控制）
        LOITER,                      // 所有轴都在保持位置
        CONTROLLER_TO_PILOT_OVERRIDE // 飞行员对此轴进行了输入控制，并且此轴正在过渡到飞行员覆盖模式（如果没有飞行员输入，其他轴将过渡到刹车模式）
    };
    ...
    // 与飞行员输入相关的变量
    float pilot_roll;  // 飞行员所要求的滚转角度（经过滤波处理，缓慢回归零度）
    float pilot_pitch; // 飞行员所要求的俯仰角度（经过滤波处理，缓慢回归零度）

    // 与刹车（减速）相关的变量
    struct {
        uint8_t time_updated_roll : 1;  // 一旦我们重新估计了刹车时间便为真。当飞行器在刹车后开始平稳时进行一次
        uint8_t time_updated_pitch : 1; // 同上，用于俯仰角

        float   gain;            // 在刹车期间，用于将飞行器的速度转换为倾斜角度的增益（根据速率计算）
        float   roll;            // 在刹车期间的目标滚转角度
        float   pitch;           // 在刹车期间的目标俯仰角度
        int16_t timeout_roll;    // 允许完成刹车的周期数，此超时将在半刹车时更新
        int16_t timeout_pitch;   // 同上，用于俯仰角
        float   angle_max_roll;  // 刹车期间达到的最大倾斜角。用于确定飞行器何时开始平稳，以便我们可以重新估计刹车时间
        float   angle_max_pitch; // 刹车期间达到的最大倾斜角。用于确定飞行器何时开始变平，以便我们可以重新评估刹车时间
        int16_t to_loiter_timer; // 在POSHOLD_TO_LOITER中混合刹车和悬停控制的周期数
    } brake;

    // 与悬停相关的变量
    int16_t controller_to_pilot_timer_roll;  // 在POSHOLD_CONTROLLER_TO_PILOT中混合控制器和飞行员控制的周期数
    int16_t controller_to_pilot_timer_pitch; // 同上，用于俯仰角
    float   controller_final_roll;           // 当我们退出刹车或悬停模式时，控制器的最终滚转角度（用于与飞行员输入混合）
    float   controller_final_pitch;          // 当我们退出刹车或悬停模式时，控制器的最终俯仰角度（用于与飞行员输入混合）

    // 与风补偿相关的变量
    Vector2f wind_comp_ef;          // 地面框架中的风补偿，位置控制器的滤波倾斜角
    float    wind_comp_roll;        // 用于风补偿的滚转角度
    float    wind_comp_pitch;       // 用于风补偿的俯仰角度
    uint16_t wind_comp_start_timer; // 在悬停启动后，延迟开始风补偿的计数器
    int8_t   wind_comp_timer;       // 计数器，将风补偿滚动/俯仰倾斜角计算减少到10hz

    // 最终输出
    float roll;  // 发送给姿态控制器的最终滚转角度
    float pitch; // 发送给姿态控制器的最终俯仰角度
};

class ModePosLoiter : public Mode {

public:
    // inherit constructor
    using Mode::Mode;
    Number mode_number() const override { return Number::POSLOITER; }
...
protected:
    const char* name() const override { return "POSLOIT"; }
    const char* name4() const override { return "PSLT"; }
