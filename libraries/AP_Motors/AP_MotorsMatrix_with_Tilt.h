/// @file	AP_MotorsMatrix_with_Tilt.h
...
/// @class      AP_MotorsMatrix_with_Tilt
class AP_MotorsMatrix_with_Tilt : public AP_MotorsMulticopter {
public:

    /// Constructor
    AP_MotorsMatrix_with_Tilt(uint16_t loop_rate, uint16_t speed_hz = AP_MOTORS_SPEED_DEFAULT) :
        AP_MotorsMulticopter(loop_rate, speed_hz)
    {
        if (_singleton != nullptr) {
            AP_HAL::panic("AP_MotorsMatrix_with_Tilt must be singleton");
        }
        _singleton = this;
    };

    // get singleton instance
    static AP_MotorsMatrix_with_Tilt *get_singleton() {
        return _singleton;
    }
    ...
    int16_t output_to_tilt(float actuator);
...
private:
    ...
    static AP_MotorsMatrix_with_Tilt *_singleton;
